...
# Simulated data
To illustrate the use of the model we simulate a dataset with two explanatory variables and 10 categories with 50 data points within each category. The two explanatory variables influence both the expectation and variance of y. 
```{r data, fig.height=7, fig.width=7}
n_cat <- 10
n <- 50
d <- data.frame(x1 = rnorm(n*n_cat, 2, 1), x2=rnorm(n*n_cat, 1, 1), cat = rep(1:n_cat, each = n))
d$y <- rnorm(nrow(d), 1+0.5*d$x1+0.1*d$x2, exp(0.5*(1+0.5*d$x1+0.6*d$x2))) + rep(rnorm(n_cat), each = n)
par(mfrow=c(2,1))
with(d, plot(x1,y))
with(d, plot(x2,y))
par(mfrow=c(1,1))
...
# The model
```{r analysis}
mod <- lmmHeterosced(formula = y~ x1c + x2c +(1|cat), heterosced_formula = ~ x1c + x2c, data = d)
```
